...
import '../../../widgets/profie_picture_widget.dart';

class MaidsAccountWidget extends ConsumerStatefulWidget {
  final MaidModel? maid;

  const MaidsAccountWidget({this.maid, super.key});

  @override
  ConsumerState<MaidsAccountWidget> createState() => _MaidsAccountWidgetState();
}

class _MaidsAccountWidgetState extends ConsumerState<MaidsAccountWidget> {
  ...
  Widget buildTextField(String label, TextEditingController controller,
      {bool isNumber = false, bool validate = true, bool isReadOnly = false}) {
    return Padding(
  ...
  void save(WidgetRef ref, BuildContext context) async {
    if (_formKey.currentState!.validate()) {
      ...
      if (widget.maid != null) {
        ref.read(currentMaidProvider.notifier).updateMaid(maidData);
      } else {
        //ref.read(currentMaidProvider.notifier).addMaid(maidData);
      }
      setState(() {
        isSaving = false;
      });
